...
A group on a GitHub forge with no `image:` and no `build:` runs `ghcr.io/actions/actions-runner:latest`, configured with `./config.sh --url ... --token ... --name ... --work ... --unattended --replace` and started with `./run.sh`. The configure step is guarded on the `.runner` file the runner writes when it registers, so it happens once and a restart goes straight to `./run.sh`. That matters because the registration token in the command expires within the hour: a restarted seat reuses the credentials it stored rather than the token, so a daemon restart, a host reboot and an `apply` that starts a stopped seat all work with a token nobody could mint again. A recreated container starts from a fresh filesystem, so it registers again against a fresh token. `--replace` lets a recreated runner take its own name back at the forge. There is no `--ephemeral`, so runners are persistent and their caches stay warm. The work directory is wiped when a runner is created and kept across restarts.
